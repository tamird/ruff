...
//!
//! Formulas are independent of the language and type system that evaluate their atoms.
...
///
/// The primitive atoms of the formula are scope-local predicates, which express some
...
/// for a particular predicate, if your formula needs to consider how a
...
pub struct InteriorNode {
    ...
    /// constraints, this represents a runtime property interpreted by the caller.
...
impl ReachabilityConstraintsBuilder {
    ...
    /// Compact the store, retaining roots passed to [`Self::mark_used`] and their descendants.
    /// Only those retained IDs may be looked up in the resulting store.
    ...
    ///
    /// We compare interior nodes by their scope-local predicate IDs, reversing the
    /// comparison. Callers typically assign these IDs sequentially
    ...
    /// separate IDs for separate occurrences, rather than interning predicates solely
    /// by their syntax.
...
mod tests {
    ...
    #[test]
    fn complementary_paths_and_compaction() {
        let mut builder = ReachabilityConstraintsBuilder::default();
        let discarded = builder.add_atom(ScopedPredicateId::from_u32(0));
        let atom = ScopedPredicateId::from_u32(1);
        let positive = builder.add_atom(atom);
        let negative = builder.add_not_constraint(positive);
        assert_eq!(builder.add_or_constraint(positive, negative), ALWAYS_TRUE);
        assert_eq!(builder.add_and_constraint(positive, negative), ALWAYS_FALSE);
        assert_eq!(builder.add_not_constraint(AMBIGUOUS), AMBIGUOUS);
        assert_ne!(discarded, positive);

        builder.mark_used(positive);
        let store = builder.build();
        assert_eq!(store.used_interiors().len(), 1);
        let node = store.get_interior_node(positive);
        assert_eq!(node.atom(), atom);
        assert_eq!(node.if_true(), ALWAYS_TRUE);
        assert_eq!(node.if_ambiguous(), AMBIGUOUS);
        assert_eq!(node.if_false(), ALWAYS_FALSE);
    }
}
