...
impl EmbeddedFilePath<'_> {
    pub(crate) fn as_str(&self) -> &str {
        match self {
            ...
            EmbeddedFilePath::AutogeneratedStarlark => "mdtest_snippet.bzl",
    ...
    fn is_allowed_explicit_path(path: &str) -> bool {
        ...
        .all(|source_type| path != EmbeddedFilePath::Autogenerated(*source_type).as_str())
            && path != EmbeddedFilePath::AutogeneratedStarlark.as_str()
    }
}

/// A single file embedded in a [`Section`] as a fenced code block.
///
/// Currently must be a Python file (`py` language), a type stub (`pyi`), a Starlark file (`bzl`),
/// a Jupyter notebook (`ipynb`) or a [typeshed `VERSIONS`] file.
///
...
///
/// A checkable embedded file makes its containing [`Section`] into a [`MarkdownTest`], and will be
...
pub struct EmbeddedFile<'s> {
    ...
    pub checkable_code_blocks: Vec<CodeBlock<'s>>,
}
...
impl EmbeddedFile<'_> {
    fn append_code(&mut self, backtick_offsets: BacktickOffsets, new_code: &str) {
        ...
        self.checkable_code_blocks.push(CodeBlock {
    ...
    pub(crate) fn is_checkable(&self) -> bool {
        matches!(self.lang, "py" | "python" | "pyi" | "bzl" | "ipynb")
    }
}
    ...
    F: FnMut(&C) -> anyhow::Result<()>,
{
    ...
    fn process_code_block(
        ...
        let path = match self.explicit_path {
            ...
            None => match lang {
                ...
                "bzl" => EmbeddedFilePath::AutogeneratedStarlark,
                "ipynb" => EmbeddedFilePath::Autogenerated(PySourceType::Ipynb),
                "" => {
                    bail!(
        ...
        match self.current_section_files.entry(path.clone()) {
            Entry::Vacant(entry) => {
                ...
                let index = self.files.push(EmbeddedFile {
                    ...
                    checkable_code_blocks: vec![CodeBlock {
                        backticks: backtick_offsets,
    ...
    fn current_section_has_merged_snippets(&self) -> bool {
        self.current_section_files
            .values()
            .any(|id| self.files[*id].checkable_code_blocks.len() > 1)
    }
    ...
    fn process_inline_snapshot(
        ...
        let code_block = file.checkable_code_blocks.last_mut().unwrap();
        ...
        if let Some(existing_block) = &code_block.inline_snapshot_block {
            ...
            bail!(
                "Checkable code block on line `{code_block_start}` has more than one `snapshot` block: first on line {existing_start} and another on line {backtick_start}.",
            );
        }
...
mod tests {
    ...
    fn starlark_file_test() {
        ...
        assert_eq!(file.path, EmbeddedFilePath::AutogeneratedStarlark);
        assert_eq!(file.lang, "bzl");
        assert_eq!(file.code, "value: int = 1");
    }
    ...
    fn notebook_test() {
        ...
        assert_eq!(
            file.path,
            EmbeddedFilePath::Autogenerated(PySourceType::Ipynb)
        );
